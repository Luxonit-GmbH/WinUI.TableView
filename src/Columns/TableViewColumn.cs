...
public abstract partial class TableViewColumn : DependencyObject
{
    ...
    /// <summary>
    /// Gets or sets whether the column's minimum width is auto-sized to fit the header and the content the FIRST time
    /// each cell renders. The minimum only ever grows and is never recomputed on data updates, so content/header is
    /// kept from being clipped without re-measuring on every change. Defaults to <see langword="false"/>.
    /// </summary>
    public bool AutoSizeMinWidth
    {
        get => (bool)GetValue(AutoSizeMinWidthProperty);
        set => SetValue(AutoSizeMinWidthProperty, value);
    }
    ...
    /// <summary>
    /// The largest header/content width seen for this column, accumulated from the FIRST render of each cell when
    /// <see cref="AutoSizeMinWidth"/> is set (never grown on data updates). Folded into the effective minimum width
    /// during header-width calculation so content/header is not clipped.
    /// </summary>
    internal double AutoMinWidth { get; private set; }

    /// <summary>
    /// Grows <see cref="AutoMinWidth"/> to fit a first-rendered content width and requests a width recalculation.
    /// </summary>
    internal void GrowAutoMinWidth(double width)
    {
        if (width > AutoMinWidth)
        {
            AutoMinWidth = width;
            OwningCollection?.HandleColumnPropertyChanged(this, nameof(MinWidth)); // reuse the MinWidth recalc path
        }
    }

    /// <summary>
    /// Whether the user has sized this column by hand, which retires the <see cref="AutoSizeMinWidth"/> floor.
    /// </summary>
    internal bool IsUserResized { get; private set; }

    /// <summary>
    /// Records that the user resized this column, so the width captured on first render stops acting as a minimum.
    /// </summary>
    /// <remarks>
    /// <see cref="AutoSizeMinWidth"/> is a first-render sizing aid — it exists so a column is not born too narrow
    /// for its header and content. Left in force it also becomes a floor the user can never drag below, which
    /// reads as "columns can be widened but not narrowed". An explicit resize is a clearer statement of intent
    /// than the captured measurement, so it wins; <see cref="MinWidth"/>, which the app sets deliberately, does not
    /// yield.
    /// </remarks>
    internal void NotifyUserResized()
    {
        if (IsUserResized)
        {
            return;
        ...
        IsUserResized = true;
        OwningCollection?.HandleColumnPropertyChanged(this, nameof(MinWidth));
    }
    ...
    private static void OnPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        if (d is TableViewColumn { OwningCollection: { } } column)
        {
            ...
            else if (e.Property == AutoSizeMinWidthProperty)
                column.OwningCollection.HandleColumnPropertyChanged(column, nameof(MinWidth));
    ...
    /// <summary>
    /// Identifies the AutoSizeMinWidth dependency property.
    /// </summary>
    public static readonly DependencyProperty AutoSizeMinWidthProperty = DependencyProperty.Register(nameof(AutoSizeMinWidth), typeof(bool), typeof(TableViewColumn), new PropertyMetadata(false, OnPropertyChanged));
